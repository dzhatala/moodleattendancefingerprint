...
public class JDKAsyncHttpProvider implements AsyncHttpProvider {
	private final static Logger logger = LoggerFactory
			.getLogger(JDKAsyncHttpProvider.class);
	...
	public JDKAsyncHttpProvider(AsyncHttpClientConfig config) {
		...
		this.config = config;
		AsyncHttpProviderConfig<?, ?> providerConfig = config
	...
	private void configure(JDKAsyncHttpProviderConfig config) {
		...
		if (config
				.getProperty(JDKAsyncHttpProviderConfig.FORCE_RESPONSE_BUFFERING) != null) {
	...
	public <T> ListenableFuture<T> execute(Request request,
			AsyncHandler<T> handler, ListenableFuture<?> future)
			throws IOException {
		if (isClose.get()) {
		...
		if (config.getMaxTotalConnections() > -1
				&& (maxConnections.get() + 1) > config.getMaxTotalConnections()) {
			throw new IOException(String.format("Too many connections %s",
					config.getMaxTotalConnections()));
		}

		ProxyServer proxyServer = ProxyUtils.getProxyServer(config, request);
		Realm realm = request.getRealm() != null ? request.getRealm() : config
				.getRealm();
		Proxy proxy = null;
		...
		int requestTimeout = (conf != null && conf.getRequestTimeoutInMs() != 0) ? conf
				.getRequestTimeoutInMs() : config.getRequestTimeoutInMs();

		JDKDelegateFuture delegate = null;
		if (future != null) {
			delegate = new JDKDelegateFuture(handler, requestTimeout, future,
					urlConnection);
		}

		JDKFuture f = delegate == null ? new JDKFuture<T>(handler,
				requestTimeout, urlConnection) : delegate;
		f.touch();

		f.setInnerFuture(config.executorService().submit(
				new AsyncHttpUrlConnection(urlConnection, request, handler, f)));
	...
	private HttpURLConnection createUrlConnection(Request request)
		...
		HttpURLConnection urlConnection = (HttpURLConnection) request.getURI()
	...
	private final class AsyncHttpUrlConnection<T> implements Callable<T> {
		...
		public AsyncHttpUrlConnection(HttpURLConnection urlConnection,
		...
		public T call() throws Exception {
			...
			try {
				URI uri = null;
				// Encoding with URLConnection is a bit bogus so we need to try
				// both way before setting it
				try {
				...
				if (asyncHandler instanceof TransferCompletionHandler) {
					throw new IllegalStateException(
							TransferCompletionHandler.class.getName()
				...
				ResponseStatus status = new ResponseStatus(uri, urlConnection,
						JDKAsyncHttpProvider.this);
				FilterContext fc = new FilterContext.FilterContextBuilder()
						.asyncHandler(asyncHandler).request(request)
				...
				boolean redirectEnabled = (request.isRedirectEnabled() || config
						.isRedirectEnabled());
				if (redirectEnabled && (statusCode == 302 || statusCode == 301)) {

					if (currentRedirectCount++ < config.getMaxRedirects()) {
						String location = urlConnection
								.getHeaderField("Location");
						URI redirUri = AsyncHttpProviderUtils.getRedirectUri(
								uri, location);
				...
				Realm realm = request.getRealm() != null ? request.getRealm()
						: config.getRealm();
				if (statusCode == 401 && !isAuth.getAndSet(true)
						&& realm != null) {
					String wwwAuth = urlConnection
							.getHeaderField("WWW-Authenticate");

					logger.debug("Sending authentication to {}",
							request.getUrl());
				...
				if (state == AsyncHandler.STATE.CONTINUE) {
					state = asyncHandler.onHeadersReceived(new ResponseHeaders(
							uri, urlConnection, JDKAsyncHttpProvider.this));
				}
				...
				if (state == AsyncHandler.STATE.CONTINUE) {
					...
					if (bufferResponseInMemory || byteToRead <= 0) {
						...
						byte[] bytes = AsyncHttpProviderUtils.readFully(is,
								lengthWrapper);
						stream = new ByteArrayInputStream(bytes, 0,
								lengthWrapper[0]);
					...
					if (byteToRead > 0) {
						...
						while (leftBytes > -1) {
							...
							leftBytes -= read;
							asyncHandler
									.onBodyPartReceived(new ResponseBodyPart(
											uri, b, JDKAsyncHttpProvider.this,
											leftBytes > -1));
						}
					}

					if (request.getMethod().equalsIgnoreCase("HEAD")) {
						asyncHandler.onBodyPartReceived(new ResponseBodyPart(
								uri, "".getBytes(), JDKAsyncHttpProvider.this,
								true));
					}
				}
				...
				if (t instanceof IOException
						&& !config.getIOExceptionFilters().isEmpty()) {
					FilterContext fc = new FilterContext.FilterContextBuilder()
							.asyncHandler(asyncHandler).request(request)
			...
			if (t instanceof UnknownHostException) {
				...
				if (request.getPerRequestConfig() != null
						&& request.getPerRequestConfig()
								.getRequestTimeoutInMs() != -1) {
					responseTimeoutInMs = request.getPerRequestConfig()
							.getRequestTimeoutInMs();
				}
				t = new TimeoutException(String.format(
						"No response received after %s", responseTimeoutInMs));
		...
		private void configure(URI uri, HttpURLConnection urlConnection,
				Request request) throws IOException, AuthenticationException {
			...
			boolean avoidProxy = ProxyUtils.avoidProxy(proxyServer,
					uri.getHost());
			if (!avoidProxy) {
			...
			if (request.getHeaders().getFirstValue("User-Agent") != null) {
				urlConnection.setRequestProperty("User-Agent", request
						.getHeaders().getFirstValue("User-Agent"));
				...
				urlConnection
						.setRequestProperty(
								...
								AsyncHttpProviderUtils
										.constructUserAgent(JDKAsyncHttpProvider.class));
			}

			if (isNonEmpty(request.getCookies())) {
				urlConnection.setRequestProperty("Cookie",
						AsyncHttpProviderUtils.encodeCookies(request
								.getCookies()));
			}
			...
			if ("POST".equals(reqType) || "PUT".equals(reqType)) {
				...
				if (cachedBytes != null) {
					urlConnection.setRequestProperty("Content-Length",
							String.valueOf(cachedBytesLenght));
					urlConnection
							.setFixedLengthStreamingMode(cachedBytesLenght);
					urlConnection.getOutputStream().write(cachedBytes, 0,
							cachedBytesLenght);
				} else if (request.getByteData() != null) {
					urlConnection.setRequestProperty("Content-Length",
							String.valueOf(request.getByteData().length));
					urlConnection.setFixedLengthStreamingMode(request
							.getByteData().length);

					urlConnection.getOutputStream()
							.write(request.getByteData());
				} else if (request.getStringData() != null) {
					if (!request.getHeaders().containsKey("Content-Type")) {
						urlConnection.setRequestProperty("Content-Type",
								"text/html;" + bodyCharset);
					}
					...
					int[] lengthWrapper = new int[1];
					cachedBytes = AsyncHttpProviderUtils.readFully(
					...
					for (final Map.Entry<String, List<String>> paramEntry : request
							.getParams()) {
					...
					urlConnection.getOutputStream().write(
							sb.toString().getBytes(bodyCharset));
					...
					urlConnection.setFixedLengthStreamingMode((int) file
							.length());
	...
	private Proxy configureProxyAndAuth(final ProxyServer proxyServer,
			final Realm realm) throws AuthenticationException {

		Proxy proxy = null;
		if (proxyServer != null) {

			String proxyHost = proxyServer.getHost().startsWith("http://") ? proxyServer
					.getHost().substring("http://".length()) : proxyServer
					.getHost();
		...
		final boolean hasProxy = (proxyServer != null && proxyServer
				.getPrincipal() != null);
		final boolean hasAuthentication = (realm != null && realm
				.getPrincipal() != null);
		if (hasProxy || hasAuthentication) {
			...
			Authenticator.setDefault(new Authenticator() {
				protected PasswordAuthentication getPasswordAuthentication() {
					if (hasProxy
							&& getRequestingHost()
									.equals(proxyServer.getHost())
							&& getRequestingPort() == proxyServer.getPort()) {
						String password = "";
						if (proxyServer.getPassword() != null) {
							password = proxyServer.getPassword();
						}
						return new PasswordAuthentication(proxyServer
								.getPrincipal(), password.toCharArray());
					}

					if (hasAuthentication) {
						return new PasswordAuthentication(realm.getPrincipal(),
								realm.getPassword().toCharArray());
					}
