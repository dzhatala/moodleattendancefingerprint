...
public class MbrolaVoiceDirectory extends VoiceDirectory {
	...
	public Voice[] getVoices() {
		...
		if (base == null || base.trim().length() == 0) {
			System.out
					.println("System property \"mbrola.base\" is undefined.  "
...
//			Voice mbrola4 = new MbrolaVoice("id1", "id1", 16f, 125F, 12F,
//					"mbrola_id1", Gender.MALE, Age.YOUNGER_ADULT,
//					"MBROLA Voice id1", Locale.US, "general", "mbrola", lexicon);

			Voice mbrola4 = new MbrolaVoice("id1", "id1", 220f, 1F, 12F,
					"mbrola_id1", Gender.MALE, Age.YOUNGER_ADULT,
					"MBROLA Voice id1", Locale.US, "general", "mbrola", lexicon);

			
			Voice mbrola5 = new MbrolaVoice("de1", "de1", 150f, 125F, 12F,
					"mbrola_de1", Gender.MALE, Age.YOUNGER_ADULT,
					"MBROLA Voice de1", Locale.GERMAN, "general", "mbrola", lexicon);

			
			// Voice[] voices = {mbrola1, mbrola2, mbrola3};
			Voice[] voices = { mbrola1, mbrola2, mbrola3, mbrola4 , mbrola5};

			List validVoices = new java.util.ArrayList();
			int count = 0;

			for (int i = 0; i < voices.length; i++) {
				MbrolaVoiceValidator validator = new MbrolaVoiceValidator(
						(MbrolaVoice) voices[i]);
				try {
					validator.validate();
			...
			if (count == 0) {
				...
				if (base.indexOf('~') != -1) {
					System.err
							.println("DO NOT USE ~ as part of the path name\n"
